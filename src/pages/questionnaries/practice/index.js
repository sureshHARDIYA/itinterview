...
class Questionnaries extends Component {
  ...
  render() {
    ...
    const preQuestion =
      data && this.getListedData(data.questionnaireFind || {});
        ...
        <div className="questionApp">
          ...
          {this.state.activeView === "submitDialog" && (
            <SubmitResult
              ...
              cancelResults={this.showQuizQuestion.bind(
                this,
  ...
  submitAnswer(answer, num) {
    var app = this;
    if (
      this.state.answers[app.state.currentQuestionIndex] &&
      app.state.currentQuestionIndex + 1 < num
    ) {
  ...
  submitBack() {
    ...
    window.setTimeout(function() {
      ...
      hasPrevStep
        ? app.showQuizQuestion(prevIndex)
        : (window.location.href = window.localStorage.getItem("cntURL"));
    }, 100);
  }
...
class Quizinator extends Component {
  ...
  onCheckboxChange = checkedValues => {
    if (
      this.props.cntAnswer[this.props.currentQuestionIndex] ||
      this.state.seted
    ) {
      return;
    }
    ...
    let rightAnswer = [],
      answers = this.props.quiz.questions[this.props.currentQuestionIndex]
        .answers;
    ...
    for (let i = 0; i < checkedValues.length; i++) {
      if (
        !rightAnswer.includes(checkedValues[i]) ||
        eqSet(checkedValues, rightAnswer)
      ) {
  ...
  onNextHandle = event => {
    ...
    let isCor = true,
      scroing = this.props.quiz.questions[this.props.currentQuestionIndex]
        .answers;
    ...
    let answer = {
        value: 1,
        isCorrect: isCor,
        qsChecked: this.state.qsChecked
      },
  ...
  render() {
              ...
              <div className="checkbox" key={i}>
                <Checkbox value={i + 1} checked={false}>
                  ...
                  <img
                    ...
                    className={
                      this.props.cntAnswer[this.props.currentQuestionIndex] &&
                      this.props.cntAnswer[
                        this.props.currentQuestionIndex
                      ].qsChecked.includes(i + 1)
                        ? "showCheck"
                        : "hideCheck"
                    }
                    src={
    ...
    return (
      <section
        className={
          "quizSection" + (this.props.buttonsDisabled ? " transitionOut" : "")
        }
      >
        <div className="helpImg">
          <img
            src={require("../../../img/repeat.png")}
        ...
        <Progress
          percent={Math.round(
            (parseInt(this.props.currentQuestionIndex) /
              parseInt(this.props.quiz.numOfQuestions)) *
              100
          )}
          status="active"
          showInfo={false}
        ...
        >
          <p>
            <b>Explanation:</b> {question.explainAnswer}
          </p>
        </div>
        <div
          className={
...
class QuizResults extends Component {
  ...
  render() {
    ...
    let cntPercent = Math.round(
      (parseInt(numCorrect) / parseInt(this.props.quizNum)) * 100
    );
    let result =
      cntPercent > 50
        ? "You have passed this practice"
    ...
    return (
      <section className="resultsSection">
        ...
        <div className="scoring">
          You got <em>{numCorrect}</em> correct scoring a total of{" "}
          <b>{score}</b> out of a possible <b>{possibleScore}</b>.
        </div>
