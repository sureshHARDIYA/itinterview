...
class QuizDescription extends Component {
  render() {
    ...
    let htmlDescription = function() {
      return { __html: quiz.introduction };
    };
...
class Quizinator extends Component {
  ...
  onCheckboxChange = checkedValues => {
    ...
    for (
      let i = 1;
      i <=
      this.props.quiz.questions[this.props.currentQuestionIndex].answers.length;
      i++
    ) {
      let id = "check" + i,
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (
          ...
          this.props.quiz.questions.length - 1
        ) {
          for (
            let i = 1;
            i <=
            this.props.quiz.questions[this.props.currentQuestionIndex].answers
              .length;
            i++
          ) {
            let id = "check" + i,
...
class QuizResults extends Component {
  render() {
    ...
    const results = this.props.results.map((item, i) => {
      ...
      let explanationHtml = function() {
        return { __html: item.explanation };
      };
      let response =
        item.isCorrect === true
